...
def train_model(model, train_loader, val_loader, classes, save_dir, num_epochs=50, learning_rate=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if batch_idx % 5 == 0:  # RSSCN7数据集较小，更频繁显示进度
        ...
        if val_acc > best_val_acc:
            ...
            print(f'🎉 新的最佳模型! 验证准确率: {best_val_acc:.2f}% (提升 {improvement:.2f}%)')
...
def load_model(model_path, device=None):
    ...
    print(f"模型加载成功!")
    print(f"  数据集: {checkpoint.get('dataset', 'Unknown')}")
...
def main():
    ...
    try:
        ...
        print(f"预期每个epoch训练批次数: {len(train_loader)}")
        print(f"预期每个epoch验证批次数: {len(val_loader)}")
    except Exception as e:
        print(f"❌ 数据集加载失败: {e}")
        return
    ...
    print(f"📊 训练历史图像已保存: {os.path.join(SAVE_DIR, 'training_history_rsscn7.png')}")
