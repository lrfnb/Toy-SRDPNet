...
class RSSCN7TestDataset(Dataset):
...
def load_model(model_path, device=None):
    ...
    print(f"✅ 模型加载成功!")
    print(f"  数据集: {checkpoint.get('dataset', 'RSSCN7')}")
...
def create_test_dataloader(data_dir, batch_size=32):
    ...
    test_dataset = RSSCN7TestDataset(data_dir, transform=test_transform)
...
def save_detailed_results(results, metrics, class_names, model_info, test_data_dir, save_dir):
    """保存详细的测试结果到txt文件"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_filename = f"rsscn7_test_results_{timestamp}.txt"
    results_path = os.path.join(save_dir, results_filename)

    with open(results_path, 'w', encoding='utf-8') as f:
        f.write("RSSCN7 Remote Sensing Scene Classification - Test Results\n")
        f.write("=" * 80 + "\n")
        f.write(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"模型文件: {model_info.get('model_path', 'Unknown')}\n")
        f.write(f"测试数据路径: {test_data_dir}\n")
        f.write(f"数据集: {model_info.get('dataset', 'RSSCN7')}\n")
        f.write(f"模型架构: {model_info.get('model_architecture', 'ResNet18')}\n")
        ...
        f.write(f"错误预测数: {results['total_samples'] - results['correct_samples']}\n")
        f.write(f"错误率: {(1 - results['accuracy']):.4f} ({(1 - results['accuracy']) * 100:.2f}%)\n")
        f.write("\n")
        ...
        f.write("各类别详细指标:\n")
        f.write("-" * 80 + "\n")
        f.write(f"{'类别':<10} {'精确率':<10} {'召回率':<10} {'F1分数':<10} {'支持数':<10} {'准确率':<10}\n")
        f.write("-" * 80 + "\n")
        ...
        for i, class_name in enumerate(class_names):
            ...
            f.write(f"'{class_name}'       {metrics['precision_per_class'][i]:<10.4f} "
        ...
        f.write("总计\n")
        f.write("-" * (12 + 8 * len(class_names) + 8) + "\n")
        ...
        for j in range(len(class_names)):
            ...
            f.write(f"{col_sum:<8}")
        f.write(f"{results['total_samples']:<8}\n")
        ...
        if error_by_class:
            for true_class, errors in sorted(error_by_class.items()):
                f.write(f"\n真实类别 '{true_class}' 的错误预测 ({len(errors)} 个):\n")
                f.write("-" * 60 + "\n")
                f.write(f"{'文件名':<25} {'预测为':<10} {'置信度':<10}\n")
                f.write("-" * 45 + "\n")

                # 按置信度排序，显示前10个
                errors.sort(key=lambda x: x['confidence'], reverse=True)
                for error in errors[:10]:
                    f.write(f"{error['filename'][:25]:<25} '{error['predicted']}'     {error['confidence']:<10.4f}\n")

                if len(errors) > 10:
                    f.write(f"... (还有 {len(errors) - 10} 个错误预测)\n")
        else:
            f.write("🎉 所有样本都预测正确！\n")
...
def save_confusion_matrix_plot(confusion_matrix, class_names, save_dir):
    ...
    plt.title('Confusion Matrix - RSSCN7 Scene Classification', fontsize=16, fontweight='bold')
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plot_path = os.path.join(save_dir, f"rsscn7_confusion_matrix_{timestamp}.png")
...
def save_results_csv(results, class_names, save_dir):
    """保存详细结果到CSV文件"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_path = os.path.join(save_dir, f"rsscn7_detailed_predictions_{timestamp}.csv")
...
def main():
    # 配置参数
    MODEL_PATH = "/data2/lrf/HIIF/models/cls/results/resnet18_rsscn7_best.pth"
    TEST_DATA_DIR = "/data2/lrf/ESRGAN/results_rss_x8"
    SAVE_DIR = "/data2/lrf/HIIF/models/cls/results/esrgan"
    BATCH_SIZE = 32

    print("🧪 RSSCN7遥感场景分类 - 模型测试")
    print("=" * 80)
    ...
    try:
        ...
        model_info = {
            ...
            'dataset': checkpoint.get('dataset', 'RSSCN7'),
        ...
        # 保存详细结果
        print(f"\n💾 保存测试结果...")
        results_path = save_detailed_results(results, metrics, class_names, model_info, TEST_DATA_DIR, SAVE_DIR)
        ...
        # 打印总结
        print(f"\n" + "=" * 80)
        print(f"🎉 RSSCN7测试完成! 结果总结:")
        print(f"📈 测试准确率: {results['accuracy']:.2f}%")
        print(f"📊 F1分数 (宏平均): {metrics['f1_macro']:.4f}")
        print(f"📊 F1分数 (加权平均): {metrics['f1_weighted']:.4f}")
        print(f"🔢 总样本数: {results['total_samples']}")
        print(f"✅ 正确预测: {results['correct_samples']}")
        print(f"❌ 错误预测: {results['total_samples'] - results['correct_samples']}")
