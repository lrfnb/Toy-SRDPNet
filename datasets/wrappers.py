...
class SRImplicitDownsampledFast(Dataset):
    ...
    def __init__(self, dataset, inp_size=None, scale_min=1, scale_max=None,
        ...
        self.scale_max = scale_max
        self.augment = augment
        self.sd_features_cache = torch.load("./results/sd_features_cache.pt")
    ...
    def __getitem__(self, idx):
        ...
        hr_coord = make_coord([h_hr, w_hr], flatten=False)
        hr_rgb = crop_hr
        latent_index = idx % 800
        sd_features = self.sd_features_cache[latent_index]['latent']
        regression = resize_fn(crop_lr, (128, 128))
        regressions = torch.cat([regression, regression, regression], dim=0)  # 9x128x128
        if self.inp_size is not None:
